...
// Only the two variants that opt into the pixel-trail carry a colour; the
// rest spell out `trailColor: undefined` so the lookup below yields one
// uniform shape instead of a union missing the key.
type VariantSpec = {
  className: string;
  font: string;
  trailColor?: string;
...
} satisfies Record<Variant, VariantSpec>;
...
} satisfies Record<Variant, Record<PreviewState, string>>;

type ButtonProps = Omit<
  ComponentPropsWithoutRef<typeof BaseButton>,
  "className" | "children" | "disabled" | "render" | "nativeButton"
> & {
  variant?: Variant;
  href?: string;
  trail?: boolean;
  disabled?: boolean;
  /** Playground only — forces a visual state without real interaction. */
...
export default function Button({
  ...
  // Matches the previous Boolean(href) && !disabled test, including the
  // empty-string case, while narrowing href to string for <Link>.
  const linkHref = href && !disabled ? href : undefined;
  ...
  return (
    <BaseButton
      {...props}
      disabled={disabled}
      nativeButton={linkHref === undefined}
      render={linkHref === undefined ? undefined : <Link href={linkHref} />}
