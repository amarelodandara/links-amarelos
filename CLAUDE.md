...
- Physics/animation state with two distinct phases is a discriminated union,
  not one shape with optional fields. See the falling/sliding/resting/sinking
  link bodies in `taste/field.ts`.
- Runtime `typeof` checks are banned (`anti-slop/no-runtime-typeof`), so data
  lives in typed TS modules checked with `satisfies`, not in JSON that would
  need decoding. See `app/data/links.ts`.
